...
const resolvedSectors = sectors.map((sector) => {
  const coords = findPointsForSegment(
    routeData,
    sector.startMi,
    sector.lengthMi,
  );
...
resolvedSectors.forEach((s) =>
  console.log(
    `  ${s.name.padEnd(20)} start=(${s.lat.toFixed(5)}, ${s.lon.toFixed(5)})  track pts: ${s.track.length}`,
  ),
);
...
resolvedKoms.forEach((k) =>
  console.log(
    `  ${k.name.padEnd(20)} start=(${k.lat.toFixed(5)}, ${k.lon.toFixed(5)})  track pts: ${k.track.length}`,
  ),
);
...
resolvedRestocks.forEach((r) =>
  console.log(
    `  ${r.name.padEnd(32)} (${r.lat.toFixed(5)}, ${r.lon.toFixed(5)})`,
  ),
);
